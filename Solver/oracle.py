...
def train_model(net, train_loader, pth_filename, num_epochs, val_loader = None):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            targets = torch.nn.functional.one_hot(targets.to(torch.int64), num_classes = 2)
            optimizer.zero_grad()
        ...
        # print((inputs[targets.argmax(1)==0]))
        scheduler.step()
...
def main():
    ...
    model.load_state_dict(torch.load(oracledir))
    train_loader, val_loader = data_oracle7(batch_size, num = 250000)
    
    #### Model training (if necessary)
    train_model(model, train_loader, "oracle7x7", 71, val_loader = val_loader)
